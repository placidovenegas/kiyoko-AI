...
CARGA (todo lo de Proyecto MÁS):
  ...
  ┌─ PERSONAJES Y FONDOS — SIEMPRE CARGADOS ─────────────────────────────────
  │  characters          → TODOS los del proyecto, con TODA su info:
  │                         id, name, role, visual_description, prompt_snippet,
  │                         ai_prompt_description, hair_description,
  │                         signature_clothing, accessories, color_accent
  │  character_images    → imagen de referencia de cada personaje (file_url, is_primary)
  │  backgrounds         → TODOS los del proyecto, con TODA su info:
  │                         id, name, description, prompt_snippet,
  │                         ai_prompt_description, location_type, time_of_day,
  │                         available_angles
  │
  │  REGLA: La IA SIEMPRE tiene visibles los personajes y fondos.
  │  Son la base de todo prompt y de toda asignación escena↔personaje/fondo.
  └───────────────────────────────────────────────────────────────────────────
  ...
  scene_characters      → qué personajes están asignados a CADA escena (junction)
  scene_backgrounds     → qué fondos están asignados a CADA escena (junction)
...
REGLAS DE PERSONAJES Y FONDOS EN ESTE CONTEXTO:
  1. Al CREAR ESCENAS → la IA DEBE proponer qué personajes y fondos aparecen en cada una
  2. Al CREAR PROMPTS → la IA DEBE incorporar el prompt_snippet de los personajes y fondos asignados
  3. Al MOSTRAR ESCENAS → la IA SIEMPRE indica qué personajes/fondos tiene cada escena
  4. Si la escena NO tiene personaje/fondo → la IA lo señala y sugiere asignar uno
  5. Si el proyecto NO tiene personajes/fondos → la IA sugiere crearlos ANTES de generar prompts
```
...
Usuario pulsa "Ejecutar" en ActionPlanCard
  ...
  │     → ACCIONES DEPENDIENTES (encadenadas):
  │       Cuando create_scene va seguida de assign_character/assign_background,
  │       el action plan usa placeholders como "__NEW_SCENE_1_ID__".
  │       El executor DEBE:
  │       1. Ejecutar el INSERT de la escena
  │       2. Obtener el UUID generado por Supabase
  │       3. Reemplazar "__NEW_SCENE_1_ID__" en las acciones siguientes
  │       4. Ejecutar assign_character y assign_background con el UUID real
  │
  │       Patrón de placeholders:
  │       __NEW_SCENE_{N}_ID__    → UUID de la escena N recién creada
  │       __NEW_VIDEO_SHORT_ID__  → short_id del video recién creado
  │       __NEW_CHARACTER_ID__    → UUID del personaje recién creado
  │
  │       Implementación:
  ...
  │         const result = await executeAction(action.type, resolvedData);
  │         // Si es INSERT, guardar el ID generado
  │         if (action.type.startsWith('create_') && result.id) {
  │           idMap.set(action.data._placeholder || action.type, result.id);
  │         }
  │       }
  │
...
Estilo visual: {project.style} {project.custom_style_description}

Videos ({videos.length}):
{videos.map(v => `- ${v.title} (${v.id}): ${v.platform} · ${v.video_type} · ${v.status} · ${scenes_count} escenas`)}

[PERSONAJES DISPONIBLES — {characters.length} personajes]
{Solo si context >= project. SIEMPRE visible cuando hay proyecto.}
{characters.map(c => `
PERSONAJE: ${c.name} (${c.id})
  Rol: ${c.role}
  Iniciales: ${c.initials}
  Descripción visual: ${c.visual_description}
  Pelo: ${c.hair_description}
  Ropa: ${c.signature_clothing}
  Accesorios: ${c.accessories?.join(', ')}
  Herramientas: ${c.signature_tools?.join(', ')}
  Color acento: ${c.color_accent}
  Personalidad: ${c.personality}
  🔑 Prompt snippet: "${c.prompt_snippet}"
  🔑 AI prompt description: "${c.ai_prompt_description}"
  Imagen ref: ${c.reference_image_url ? '✅ Tiene imagen' : '❌ Sin imagen'}
`)}

[FONDOS DISPONIBLES — {backgrounds.length} fondos]
{Solo si context >= project. SIEMPRE visible cuando hay proyecto.}
{backgrounds.map(b => `
FONDO: ${b.name} (${b.id}) — Código: ${b.code}
  Tipo de locación: ${b.location_type}
  Descripción: ${b.description}
  Hora del día: ${b.time_of_day}
  Ángulos disponibles: ${b.available_angles?.join(', ')}
  🔑 Prompt snippet: "${b.prompt_snippet}"
  🔑 AI prompt description: "${b.ai_prompt_description}"
  Imagen ref: ${b.reference_image_url ? '✅ Tiene imagen' : '❌ Sin imagen'}
`)}
...
Escena #{s.scene_number} — "${s.title}" (${s.id})
  ...
  👤 Personajes asignados: ${assigned_characters.length > 0
      ? assigned_characters.map(c => `${c.name} (${c.id})`).join(', ')
      : '⚠️ NINGUNO — sin personaje asignado'}
  🏙️ Fondo asignado: ${assigned_background
      ? `${assigned_background.name} (${assigned_background.id})`
      : '⚠️ NINGUNO — sin fondo asignado'}
...
[REGLAS DE PERSONAJES Y FONDOS EN PROMPTS — MUY IMPORTANTE]

1. CREAR ESCENAS:
   - Al crear cada escena, SIEMPRE indica qué personaje(s) aparecen y qué fondo usa
   - Incluye acciones assign_character y assign_background en el action plan
   - Si no hay personajes creados → ANTES de crear escenas, sugiere crear personajes
   - Si no hay fondos creados → ANTES de crear escenas, sugiere crear fondos
   - Si el usuario insiste en crear sin personajes/fondos → crea las escenas pero advierte:
     "Las escenas están sin personajes ni fondos. Los prompts serán genéricos hasta que los asignes."

2. GENERAR PROMPTS DE IMAGEN (scene_prompts, prompt_type: 'image'):
   - SIEMPRE incorpora el prompt_snippet y/o ai_prompt_description del personaje asignado
   - SIEMPRE incorpora el prompt_snippet y/o ai_prompt_description del fondo asignado
   - Estructura del prompt: [Personaje + acción] + [Fondo/locación] + [Iluminación/mood] + [Estilo visual del proyecto]
   - Ejemplo con personaje "Laura" (prompt_snippet: "young woman 22-27, wavy blonde hair, bright smile")
     y fondo "Playa amanecer" (prompt_snippet: "tropical beach, golden morning light, calm ocean"):
     → "Young woman 22-27, wavy blonde hair, bright smile, stretching in beach hammock,
        tropical beach, golden morning light, calm ocean, warm cinematic tones, lifestyle photography"

3. GENERAR PROMPTS DE VIDEO (scene_video_clips):
   - prompt_image_first_frame: Usa el prompt de imagen + composición estática exacta
   - prompt_video: Describe movimiento MANTENIENDO la coherencia visual del personaje y fondo
   - Incluye el movimiento de cámara de scene_camera si existe

4. MOSTRAR ESCENAS AL USUARIO:
   - SIEMPRE muestra qué personaje(s) y fondo(s) tiene cada escena
   - Si una escena NO tiene personaje → marcar con ⚠️
   - Si una escena NO tiene fondo → marcar con ⚠️
   - Ejemplo en SCENE_PLAN: cada escena debe listar "Personaje: Laura · Fondo: Playa amanecer"

5. COHERENCIA:
   - Si el personaje tiene reference_image_url, mencionarlo como referencia visual
   - Si el fondo tiene available_angles, usar el ángulo adecuado para la escena
   - Si el fondo tiene time_of_day, respetar la hora del día en el prompt
   - Si el proyecto tiene global_prompt_rules, aplicarlas siempre
...
- prompt_image_first_frame: Describe la IMAGEN ESTÁTICA que será el primer fotograma
  DEBE INCLUIR: prompt_snippet del personaje + pose/acción + prompt_snippet del fondo + iluminación + estilo
- prompt_video: Describe el MOVIMIENTO que ocurre a partir de esa imagen
  DEBE INCLUIR: acción del personaje + movimiento de cámara + duración + elementos dinámicos
- Ambos prompts deben ser COHERENTES entre sí y con los datos del personaje/fondo
...
{SI characters.length === 0 Y backgrounds.length === 0}
  Antes de generar escenas, te recomiendo crear al menos un personaje y un fondo.
  Sin ellos, los prompts serán genéricos y no podremos mantener consistencia visual.

  [OPTIONS]
  ["👤 Crear personaje primero", "🏙️ Crear fondo primero", "⏩ Generar sin personajes/fondos (genérico)"]
  [/OPTIONS]

{SI characters.length === 0 PERO backgrounds.length > 0}
  Tengo {backgrounds.length} fondo(s) disponibles pero ningún personaje.
  ¿Quieres crear un personaje antes o generar escenas sin personajes?

  [OPTIONS]
  ["👤 Crear personaje primero", "⏩ Generar sin personajes"]
  [/OPTIONS]

{SI characters.length > 0 Y backgrounds.length > 0 — FLUJO NORMAL}
  Voy a generar escenas para "{video.title}" ({video.target_duration_seconds}s · {video.platform}).

  Personajes disponibles:
  {characters.map(c => `👤 ${c.name} — ${c.role} · ${c.visual_description}`)}

  Fondos disponibles:
  {backgrounds.map(b => `🏙️ ${b.name} — ${b.description}`)}
...
Cada escena tiene personaje y fondo asignados:

[SCENE_PLAN]
[
  {"scene_number":1, "title":"Amanecer en la playa", "duration":5, "arc_phase":"hook",
   "description":"Laura despierta en su hamaca al sol de la mañana",
   "character":"Laura", "character_id":"uuid-laura",
   "background":"Playa amanecer", "background_id":"uuid-playa-amanecer"},
  {"scene_number":2, "title":"El descubrimiento", "duration":6, "arc_phase":"build",
   "description":"Laura revisa su móvil y descubre los descuentos",
   "character":"Laura", "character_id":"uuid-laura",
   "background":"Playa amanecer", "background_id":"uuid-playa-amanecer"},
  {"scene_number":3, "title":"La selección", "duration":7, "arc_phase":"build",
   "description":"Laura prueba 3 outfits de playa en la boutique",
   "character":"Laura", "character_id":"uuid-laura",
   "background":"Boutique playa", "background_id":"uuid-boutique"},
  {"scene_number":4, "title":"La pasarela", "duration":7, "arc_phase":"peak",
   "description":"Laura camina por la orilla luciendo el outfit elegido",
   "character":"Laura", "character_id":"uuid-laura",
   "background":"Playa atardecer", "background_id":"uuid-playa-atardecer"},
  {"scene_number":5, "title":"El cierre", "duration":5, "arc_phase":"close",
   "description":"Laura mira a cámara, logo y claim 'Hasta -50%'",
   "character":"Laura", "character_id":"uuid-laura",
   "background":"Playa atardecer", "background_id":"uuid-playa-atardecer"}
]
[/SCENE_PLAN]
...
👤 Laura aparece en las 5 escenas.
🏙️ Fondos usados: Playa amanecer (esc. 1-2), Boutique playa (esc. 3), Playa atardecer (esc. 4-5).

[ACTION_PLAN]
{
  "description": "Crear 5 escenas con personaje y fondo asignados para 'Verano Sin Límites'",
  "requires_confirmation": true,
  "actions": [
    {
      "type": "create_scene",
    ...
    },
    {
      ...
      "data": {
        "scene_id": "__NEW_SCENE_1_ID__",
        "background_id": "uuid-playa-amanecer",
        "is_primary": true,
        "time_of_day": "morning",
        "angle": "wide"
      }
...
Nota: El action plan incluye las 5 escenas con sus asignaciones de personaje y fondo.
Aquí muestro solo las 2 primeras como ejemplo del patrón: create_scene → assign_character → assign_background.

[OPTIONS]
["Crear todas las escenas", "Revisar una por una", "Cambiar personajes/fondos asignados", "Regenerar con otro estilo", "Cancelar"]
[/OPTIONS]
```

### 7.5 Video — Generar prompts con personajes y fondos

```
👤 Usuario: Genera los prompts de imagen para todas las escenas

🤖 Kiyoko:
Voy a generar prompts de imagen para las 5 escenas.
Uso los datos de cada personaje y fondo asignado como base:

👤 Laura → "young woman 22-27, wavy blonde hair, bright smile, beach outfit"
🏙️ Playa amanecer → "tropical beach, golden morning light, calm ocean, soft horizon"
🏙️ Boutique playa → "colorful beach boutique interior, summer clothing racks, bright natural light"
🏙️ Playa atardecer → "beach at golden hour, warm orange sky, ocean waves, cinematic light"

[PROMPT_PREVIEW:image]
{"text": "Young woman 22-27, wavy blonde hair, bright smile, stretching awake in beach hammock, tropical beach, golden morning light, calm ocean, warm cinematic tones, lifestyle film photography, Pixar-inspired lighting", "version": 1, "tags": ["close_up", "golden_hour", "lifestyle", "laura", "playa_amanecer"], "scene": "Escena 1 — Amanecer en la playa"}
[/PROMPT_PREVIEW]

[PROMPT_PREVIEW:image]
{"text": "Young woman 22-27, wavy blonde hair, sitting on beach towel looking at phone screen with excited expression, tropical beach morning, natural sunlight, candid lifestyle moment, sale notification on screen", "version": 1, "tags": ["medium_shot", "morning", "lifestyle", "laura", "playa_amanecer"], "scene": "Escena 2 — El descubrimiento"}
[/PROMPT_PREVIEW]

... (3 más)

Cada prompt incluye los datos visuales de Laura y del fondo correspondiente.

[ACTION_PLAN]
{
  "description": "Crear prompts de imagen para las 5 escenas basados en personajes y fondos",
  "requires_confirmation": true,
  "actions": [
    {
      "type": "create_prompt",
      "table": "scene_prompts",
      "data": {
        "scene_id": "uuid-escena-1",
        "prompt_text": "Young woman 22-27, wavy blonde hair, bright smile, stretching awake in beach hammock, tropical beach, golden morning light, calm ocean, warm cinematic tones, lifestyle film photography",
        "prompt_type": "image",
        "is_current": true,
        "version": 1
      }
    },
    {
      ...
      "table": "scene_prompts",
      "data": {
        "scene_id": "uuid-escena-2",
        "prompt_text": "Young woman 22-27, wavy blonde hair, sitting on beach towel looking at phone screen with excited expression, tropical beach morning, natural sunlight, candid lifestyle moment",
        "prompt_type": "image",
        "is_current": true,
        "version": 1
      }
...
[SUGGESTIONS]
["Ahora generar prompts de video", "Ajustar un prompt específico", "Cambiar personaje de una escena"]
[/SUGGESTIONS]
```

### 7.6 Video — Petición ambigua: "cambia el prompt"
...
### 7.7 Escena — Mejorar prompt con variantes
...
### 7.8 Flujo "Cambiar" — Decisión en cascada
