...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-16rem)] flex-col">
      ...
      {/* Messages area */}
      <div className="flex-1 overflow-y-auto rounded-xl bg-surface-secondary p-4">
        {messages.length === 1 && messages[0] === INITIAL_MESSAGE ? (
          <div className="flex h-full flex-col items-center justify-center">
            <div className="mb-4 rounded-full bg-brand-500/10 p-4">
              <span className="text-3xl">💬</span>
            </div>
            <p className="mb-6 max-w-md text-center text-sm text-foreground-secondary">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {QUICK_SUGGESTIONS.map((suggestion) => (
                <button
                  ...
                  className="rounded-lg border border-surface-tertiary px-3 py-1.5 text-sm text-foreground-secondary transition hover:bg-surface-tertiary"
                >
                  {suggestion}
                </button>
              ))}
      ...
      <div className="mt-4 flex gap-2">
        <input
          ...
          disabled={isLoading}
          className="flex-1 rounded-xl border border-surface-tertiary bg-surface-secondary px-4 py-3 text-sm text-foreground placeholder:text-foreground-muted focus:border-brand-500 focus:outline-none disabled:opacity-50"
        />
        <button
          onClick={() => sendMessage()}
