...
import { SectionLabel } from './shared/NavItem';
      ...
      <div className="px-2 py-1">
        <Popover>
          ...
          <Popover.Content className="w-56 p-1">
            ...
            {favorites.map((fav) => (
              <Link key={fav.id} href={`/project/${fav.short_id}`} className={cn(
                'flex w-full items-center gap-2 rounded-md px-2 h-8 text-[13px] transition-colors',
                pathname.startsWith(`/project/${fav.short_id}`) ? 'bg-accent font-medium text-foreground' : 'text-foreground/80 hover:bg-accent',
              )}>
  ...
  return (
    <div className="px-1.5 py-1">
      ...
      <ul className="flex flex-col gap-0.5">
        {visible.map((fav) => {
          const isActive = pathname.startsWith(`/project/${fav.short_id}`);
          return (
            <li key={fav.id} className="group/fav relative">
              <Link href={`/project/${fav.short_id}`} className={cn(
              ...
              <div className="absolute right-1 top-0 bottom-0 flex items-center opacity-0 group-hover/fav:opacity-100 transition-opacity z-10">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button type="button" onClick={(e) => e.preventDefault()} className="flex items-center justify-center size-6 rounded text-sidebar-foreground/50 hover:text-sidebar-foreground hover:bg-sidebar-accent transition-colors cursor-pointer">
                      <MoreHorizontal className="h-3.5 w-3.5" />
                    </button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent side="right" align="start" className="w-48">
                    <DropdownMenuItem asChild><Link href={`/project/${fav.short_id}`}><ExternalLink className="mr-2 h-3.5 w-3.5" /> Abrir</Link></DropdownMenuItem>
                    <DropdownMenuItem onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/project/${fav.short_id}`); toast.success('Link copiado'); }}><Link2 className="mr-2 h-3.5 w-3.5" /> Copiar link</DropdownMenuItem>
                    <DropdownMenuItem onClick={() => router.push(`/project/${fav.short_id}/settings/sharing`)}><Share2 className="mr-2 h-3.5 w-3.5" /> Compartir</DropdownMenuItem>
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={() => toggleFavorite(fav.id)}><Star className="mr-2 h-3.5 w-3.5 fill-yellow-500 text-yellow-500" /> Quitar de favoritos</DropdownMenuItem>
                    <DropdownMenuSeparator />
